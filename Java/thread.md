...
volatile 변수는 읽기 및 쓰기 작업은 즉시 메인 메모리에서 읽어오고 저장하는 방식으로 동작합니다. 스레드가 변경한 값을 즉시 메모리에 기록하기에 CPU 캐시에만 변경된 값이 쓰여 다른 스레드가 변수의 최신 값을 볼 수 없는 문제, 즉 가시성 문제를 해결할 수 있습니다.
